...
class JAXStitchHMM:
        ...
        *,
        ...
        try:
            ...
            if self.config.ploidy_mode == "pseudo_haploid":
                hap = self._forward_backward_numpy_haploid(log_emission, switch, k)
            else:
    ...
    def _initialize_transition_factorization(
        ...
        *,
        ...
        if self.config.transition_model != "factorized":
        ...
        if self.backend == "jax" and jax is not None and jnp is not None:
            ...
            if (not has_fragments) or can_use_jax_fragments:
                ...
                ref_jax = jnp.asarray(ref_obs, dtype=jnp.float32)
                alt_jax = jnp.asarray(alt_obs, dtype=jnp.float32)
                other_jax = jnp.asarray(other_obs, dtype=jnp.float32)
                ...
                if can_use_jax_fragments:
                    ...
                    if self.config.ploidy_mode == "pseudo_haploid":
                        fn = self._get_jax_haploid_fragment_count_output_callable(
                            ...
                            ref_count.shape[1],
                            k,
                            int(fragment_center_idx.shape[0]),
                            int(fragment_obs_pos_idx.shape[0]),
                        )
                    else:
                    ...
                    hap_jax, _, _ = fn(
                        ...
                        jnp.asarray(fragment_center_idx, dtype=jnp.int32),
                        jnp.asarray(fragment_obs_fragment_idx, dtype=jnp.int32),
                        jnp.asarray(fragment_obs_pos_idx, dtype=jnp.int32),
                        jnp.asarray(fragment_obs_code, dtype=jnp.int8),
                    ...
                    fn = self._get_jax_haploid_count_output_callable(ref_count.shape[0], ref_count.shape[1], k)
                    hap_jax, _, _ = fn(
                        ref_jax,
                        alt_jax,
                        other_jax,
                    ...
                    fn = self._get_jax_diploid_count_output_callable(ref_count.shape[0], ref_count.shape[1], k)
                    hap_jax, _, _ = fn(
                        ref_jax,
                        alt_jax,
                        other_jax,
                ...
                if ref_count.shape[1] < 2:
                    counts_jax = jnp.zeros((k, k), dtype=jnp.float32)
                else:
                    counts_jax = jnp.einsum("spi,spj->ij", hap_jax[:, :-1, :], hap_jax[:, 1:, :])
                return self._fit_transition_factors_from_transition_counts_jax(
                    counts_jax,
                    k=k,
                    n_transition_pairs=int(ref_count.shape[0] * max(ref_count.shape[1] - 1, 0)),
                )
        log_emission, _, founder_alt = self.emissions(
            working_alt_prob,
            ref_count,
        ...
        if self.config.ploidy_mode == "pseudo_haploid":
            ...
            gamma = self._forward_backward_numpy_diploid(log_emission, switch, k)
            hap = _diploid_pair_to_haplotype_posterior(gamma)
        return self._fit_transition_factors_from_haplotype_posterior(hap, k=k)
            ...
            if self.config.ploidy_mode == "diploid":
                if self.backend == "torch":
                    ...
                    if self.backend == "jax" and jax is not None and jnp is not None:
                        ...
                        gamma = self._forward_backward_numpy_diploid_factorized(log_emission, switch, transition_factor_offdiag, k)
                    haplotype_posterior = _diploid_pair_to_haplotype_posterior(gamma)
                else:
                    gamma = self._forward_backward_numpy_diploid(log_emission, switch, k)
                    haplotype_posterior = _diploid_pair_to_haplotype_posterior(gamma)
                ...
                if self.backend == "torch":
                    ...
                    if self.backend == "jax" and jax is not None and jnp is not None:
                        ...
                        haplotype_posterior = self._forward_backward_numpy_haploid_factorized(
                            log_emission,
                            switch,
                            transition_factor_offdiag,
                            k,
