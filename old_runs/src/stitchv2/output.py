...
def write_xarray_float_zarr(
    ...
    *,
    ...
    """Persist floating STITCHV2 outputs as an xarray-compatible Zarr v2 store.
    ...
    root_attrs["zarr_content"] = "floating STITCHV2 outputs only; hard calls and labels remain in Parquet"
    ...
    if consolidated:
        warnings.warn(
            "Manual STITCHV2 Zarr writer emits xarray-compatible Zarr v2 metadata without consolidated .zmetadata.",
            RuntimeWarning,
            stacklevel=2,
        )
...
def _chunks_from_data(data: object, shape: tuple[int, ...]) -> tuple[int, ...]:
    ...
    if not chunks_attr:
        return tuple(max(1, int(size)) for size in shape)
    chunks: list[int] = []
...
def _write_float_dataarray_zarr(data_array: object, array_dir: Path) -> None:
    ...
    for starts in itertools.product(*starts_by_axis):
        ...
        (array_dir / chunk_index).write_bytes(np.ascontiguousarray(block_array).tobytes(order="C"))
...
def export_stitch_vcf_from_parquet(
    ...
    *,
    ...
    raise RuntimeError(
        "VCF export is intentionally disabled in STITCHV2. "
...
def export_stitch_bcf_from_parquet(
    ...
    *,
    ...
    warnings.warn(
        "BCF export is an interoperability path and will slow down I/O; keep primary STITCHV2 results in Parquet/Zarr.",
        RuntimeWarning,
        stacklevel=2,
    )
    run_output_dir = Path(run_output_dir)
