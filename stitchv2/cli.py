...
def _add_common_run_args(parser: argparse.ArgumentParser, *, require_n_founders: bool = True) -> None:
    ...
    parser.add_argument("--n-founders", type=int, required=require_n_founders, default=8)
    ...
    parser.add_argument("--dask-scheduler", choices=["local"], default="local", help="Dask scheduler type.")
    ...
    parser.add_argument("--read-stream-backend", choices=["auto", "python", "htslib"], default="auto")
    parser.add_argument("--io-workers", type=int, default=1)
    parser.add_argument("--htslib-threads-per-file", type=int, default=1)
    parser.add_argument("--fragment-likelihood-mode", choices=["replace", "augment"], default="augment")
    parser.add_argument(
...
def cmd_export_vcf(args: argparse.Namespace) -> None:
    summary = export_stitch_vcf_from_parquet(
        args.run_output_dir,
        args.output_vcf,
        chromosome=args.chromosome,
        include_haplotype_dosage=args.include_haplotype_dosage,
        gp_precision=args.gp_precision,
        ds_precision=args.ds_precision,
        hd_precision=args.hd_precision,
        bgzip=args.bgzip,
        tabix_index=args.tabix_index,
        threads=args.threads,
    )
    summary_path = Path(args.run_output_dir) / "export_vcf_summary.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tune_p.add_argument("--read-stream-backend", choices=["auto", "python", "htslib"], default="auto")
    tune_p.add_argument("--io-workers", type=int, default=1)
    tune_p.add_argument("--htslib-threads-per-file", type=int, default=1)
    tune_p.add_argument("--fragment-likelihood-mode", choices=["replace", "augment"], default="augment")
    ...
    vcf_p = sub.add_parser("export-vcf", help="Export run parquet outputs to STITCH-like VCF.")
    vcf_p.add_argument("--run-output-dir", required=True)
    vcf_p.add_argument("--output-vcf", required=True)
    vcf_p.add_argument("--chromosome", required=True)
    vcf_p.add_argument("--include-haplotype-dosage", action="store_true")
    vcf_p.add_argument("--threads", type=int, default=1)
    vcf_p.add_argument("--gp-precision", type=int, default=4)
    vcf_p.add_argument("--ds-precision", type=int, default=4)
    vcf_p.add_argument("--hd-precision", type=int, default=4)
    vcf_p.add_argument("--no-bgzip", dest="bgzip", action="store_false")
    vcf_p.add_argument("--no-tabix-index", dest="tabix_index", action="store_false")
    vcf_p.set_defaults(bgzip=True, tabix_index=True)
    vcf_p.set_defaults(func=cmd_export_vcf)

    bcf_p = sub.add_parser("export-bcf", help="Export run parquet outputs to STITCH-like BCF.")
