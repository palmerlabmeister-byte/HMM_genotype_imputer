...
The package declares Python `>=3.10` in `pyproject.toml`. Core dependencies are `numpy`, `pandas`, `pyarrow`, `pysam`, `jax`, `jaxlib`, `scipy`, `xarray`, `dask[array]`, and `distributed`.
...
### Same Run With Fixed Calibration

Calibration is on by default. This command keeps fixed calibration explicit:
...
`combine_pipeline_outputs()` returns a lazy xarray dataset backed by Dask arrays and writes combined parquet files.
...
stitchv2 export-vcf \
  --run-output-dir runs/chr1 \
  --output-vcf runs/chr1/stitchv2.vcf.gz \
  --chromosome chr1 \
  --threads 4
```

VCF FORMAT fields are `GT:GP:DS` or `GT:GP:DS:HD` when `--include-haplotype-dosage` is used.
...
| `--n-founders` | required | number of founders |
...
| `--read-stream-backend` | `auto` | `auto`, `python`, or `htslib` |
| `--io-workers` | `1` | read extraction workers |
| `--htslib-threads-per-file` | `1` | htslib threads per BAM/CRAM |
| `--fragment-likelihood-mode` | `augment` | `replace` or `augment` |
...
| `--no-calibrate-genotype-posteriors` | false | disable calibration |
| `--calibration-mode` | `fixed` | `fixed` or `masked_cv` |
| `--genotype-posterior-temperature` | `0.35` | fixed calibration temperature |
| `--genotype-posterior-blend` | `0.35` | fixed calibration blend |
| `--genotype-call-mode` | `argmax` | `argmax`, `stitch_no_call`, or `quality_gated` |
...
| `--use-lightgbm-calibrator` | false | learned calibration when truth labels exist |
...
| `--founder-immutable` | false | freeze loaded founders |
...
| `stitchv2 combine` | `--run-output-dir`, `--datasets`, `--output-dir`, `--input-dir`, `--output-file`, `--compression`, `--compression-level`, `--row-group-size` |
| `stitchv2 export-vcf` | `--run-output-dir`, `--output-vcf`, `--chromosome`, `--include-haplotype-dosage`, `--threads`, precision flags, `--no-bgzip`, `--no-tabix-index` |
| `stitchv2 export-bcf` | `--run-output-dir`, `--output-bcf`, `--chromosome`, `--include-haplotype-dosage`, `--no-tabix-index` |
...
- `--write-genotype-posteriors` and `--write-genotype-calls` are set when metrics or VCF/BCF output need GP/GT
