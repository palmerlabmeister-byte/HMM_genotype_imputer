...
class PysamReadExtractor:
    def __init__(
        ...
        *,
        ...
        read_stream_backend: Literal["auto", "python", "htslib", "snp_only_bamreader", "stitch_style_bamreader"] = "auto",
        ...
        if self.read_stream_backend in {"htslib", "snp_only_bamreader", "stitch_style_bamreader"} and _extract_sample_read_stream_htslib is None:
            raise RuntimeError(
    ...
    def _extract_with_read_stream(
        ...
        *,
        ...
        if self._use_compiled_read_stream_runtime:
            ...
            if _extract_samples_read_stream_batch_htslib is not None and self.io_workers <= 1:
                return self._extract_read_stream_compiled_batch(
                    positions=positions,
    ...
    def _extract_read_stream_compiled_batch(
        ...
        *,
        ...
        (
            ...
            fragment_obs_code,
            fragment_obs_qual,
        ) = _extract_samples_read_stream_batch_htslib(
            self._bam_paths,
            self.chromosome,
            int(region_start),
            int(region_stop),
        ...
        if depth_local.shape != depth.shape:
            raise RuntimeError(f"Compiled batch extractor returned depth shape {depth_local.shape}, expected {depth.shape}.")
        ...
        return (
            sample_offsets.astype(np.int64, copy=False),
            fragment_center_idx.astype(np.int32, copy=False),
            fragment_obs_offsets.astype(np.int64, copy=False),
            fragment_obs_pos_idx.astype(np.int32, copy=False),
            fragment_obs_code.astype(np.int8, copy=False),
            fragment_obs_qual.astype(np.uint8, copy=False),
        )
