...
def _add_common_run_args(parser: argparse.ArgumentParser, *, require_n_founders: bool = True) -> None:
    ...
    parser.add_argument("--block-size", type=int, default=0, help="HMM SNP block size (0=auto from --max-mem).")
    ...
    parser.add_argument(
        "--read-stream-backend",
        choices=["auto", "python", "htslib", "snp_only_bamreader", "stitch_style_bamreader"],
        default="auto",
    )
    ...
    parser.add_argument(
        ...
        choices=["npz", "parquet_zarr"],
        default="parquet_zarr",
        help="Evidence cache storage format. parquet_zarr stores compact fragments/support in Parquet and optional dense arrays in Zarr.",
    )
