...
from .pileup import OBS_ALT, OBS_REF, ReadEvidenceBlock, _base_weight
...
class PartitionedEvidenceCache:
    ...
    def load_available(
        ...
        if manifest is None or not self._validate_manifest(manifest, block):
            return None, np.ones(requested.shape[0], dtype=bool), self.stats(block)
        ...
        if not np.any(cached_mask):
            return None, ~cached_mask, self.stats(block)
        ...
        for part_id, part_samples in grouped.items():
            part = self._read_part(self.block_dir(block), part_id, manifest)
            ...
            blocks.append(part.select_samples(idx))
        ...
        stats["partitioned_cache_cached_samples"] = int(np.count_nonzero(cached_mask))
        stats["partitioned_cache_missing_samples"] = int(np.count_nonzero(~cached_mask))
        return cached, ~cached_mask, stats
