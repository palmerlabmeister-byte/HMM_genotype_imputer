...
def _pack_plink_variant(genotypes: np.ndarray) -> bytes:
    """
    Encode one SNP worth of diploid hard calls into PLINK1 SNP-major 2-bit format.
    Mapping: 0->00, missing->01, 1->10, 2->11
    """
    n = genotypes.shape[0]
    out = bytearray((n + 3) // 4)
...
def parquet_dosage_to_plink(
    ...
    *,
    ...
    dosage_col: str = "dosage",
    default_chrom: str = "1",
    default_ref: str = "A",
    default_alt: str = "C",
) -> None:
    """
    Convert long-format dosage parquet into PLINK bed/bim/fam files.
    The parquet is expected to have one row per (sample, position).
    """
    table = ds.dataset(dosage_parquet, format="parquet").to_table(columns=[sample_col, chrom_col, pos_col, dosage_col])
    df = table.to_pandas()
    if df.empty:
        raise ValueError("Dosage parquet is empty.")

    df[sample_col] = df[sample_col].astype(str)
    df[pos_col] = df[pos_col].astype(np.int64)
    if chrom_col not in df.columns:
        df[chrom_col] = default_chrom
    df[chrom_col] = df[chrom_col].astype(str)

    sample_ids = np.sort(df[sample_col].unique())
    variant_df = df[[chrom_col, pos_col]].drop_duplicates().sort_values([chrom_col, pos_col]).reset_index(drop=True)
    sample_index = {sample: idx for idx, sample in enumerate(sample_ids)}
    variant_index = {(row[chrom_col], int(row[pos_col])): i for i, row in variant_df.iterrows()}

    geno = np.full((sample_ids.shape[0], variant_df.shape[0]), np.nan, dtype=np.float32)
    rounded = np.rint(df[dosage_col].to_numpy(dtype=np.float32))
    rounded = np.clip(rounded, 0, 2)
    for sample, chrom, pos, g in zip(df[sample_col], df[chrom_col], df[pos_col], rounded):
        geno[sample_index[sample], variant_index[(str(chrom), int(pos))]] = g
    ...
    bim = pd.DataFrame(
        {
            "chrom": variant_df[chrom_col].astype(str).fillna(default_chrom),
            "snp": [f"{chrom}:{pos}" for chrom, pos in zip(variant_df[chrom_col], variant_df[pos_col])],
            "cm": 0.0,
            "pos": variant_df[pos_col].astype(np.int64),
            "a0": default_ref,
            "a1": default_alt,
        }
    )
    ...
    with open(f"{output_prefix}.bed", "wb") as handle:
        handle.write(bytes([0x6C, 0x1B, 0x01]))
        for variant_idx in range(geno.shape[1]):
            handle.write(_pack_plink_variant(geno[:, variant_idx]))
