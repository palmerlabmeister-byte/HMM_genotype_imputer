...
def smooth_dosage_with_pedigree(
    ...
    parent_degree = np.asarray(parent_matrix.sum(axis=1)).reshape(-1, 1)
    parent_degree[parent_degree == 0] = 1.0
    parent_mean = parent_matrix @ dosage / parent_degree
...
def apply_transmission_message_passing(
    ...
    *,
    ...
    for _ in range(max(int(iterations), 1)):
        ...
        # Keep sequenced/genotyped samples anchored to their STITCHV2 posterior.
