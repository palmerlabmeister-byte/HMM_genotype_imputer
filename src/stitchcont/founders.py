...
except Exception:  # pragma: no cover - optional at import time for HMM-only tests.
    pa = None
    pq = None
try:
    import pysam
except Exception:  # pragma: no cover - optional unless VCF founder loading is used.
    pysam = None
...
class FounderPanel:
    ...
    def to_parquet(self, path: str | Path, compression: str = "zstd") -> None:
        if pa is None or pq is None:
            raise ImportError("pyarrow is required to write FounderPanel parquet output.")
    ...
    def from_vcf(
        ...
        if pysam is None:
            raise ImportError("pysam is required for VCF founder input.")
    ...
    def from_plink(
        ...
        try:
            ...
            raise ImportError("STITCHCONT's internal PLINK loader is required for PLINK founder input.") from exc
