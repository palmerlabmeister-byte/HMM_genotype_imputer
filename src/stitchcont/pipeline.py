...
class StitchPipeline:
    def __init__(self, config: PipelineConfig):
        ...
        if str(config.snp_block_mode) not in {"exact_streaming", "exact_chunked", "independent_approx", "density_balanced_overlap"}:
            raise ValueError(
                "snp_block_mode must be one of: exact_streaming, exact_chunked, independent_approx, density_balanced_overlap."
            )
        ...
        self._output_sample_ids: np.ndarray | None = None
        self._output_positions: np.ndarray | None = None
        self._output_zarr_manifest: dict[str, object] = {}
        ...
        *,
        ...
        if cache_mode != "off" and str(self.config.compact_evidence_cache_format) != "parquet_zarr":
            raise ValueError(
                ...
                "STITCHCONT production caching stores compact 2D fragment/support data as compressed Parquet "
        ...
        if cache_mode != "off" and str(self.config.compact_evidence_cache_format) == "parquet_zarr":
            ...
            if cache_mode == "read" and np.any(missing_mask):
                missing_ids = requested_ids[missing_mask].astype(str).tolist()
                preview = ", ".join(missing_ids[:8])
                suffix = "" if len(missing_ids) <= 8 else f" ... ({len(missing_ids)} total)"
                raise RuntimeError(
                    "compact evidence cache read-only mode could not find all requested samples for "
                    f"block={int(block.block_id)} rows={int(block.row_start)}-{int(block.row_stop)}. "
                    f"Missing sample IDs: {preview}{suffix}. Build the cache first with `stitchcont evidence-build` "
                    "or rerun with --compact-evidence-cache-mode readwrite."
                )
    ...
    def _memory_cell_costs(self, founder_panel: FounderPanel, sample_ploidy: np.ndarray) -> tuple[int, int]:
        """Split the per-(sample, variant) cost into persistent and transient bytes.

        - persistent: arrays held for ALL samples of a block at once (dense read
          evidence + the output dosage/GP/haplotype matrices).  These scale with
          the full sample count and are only bounded by sample sharding.
        - transient: the forward/backward working tensors (alpha, beta, gamma,
          emission), which are bounded by the HMM sample-batch size.

        The guard and the serial sample-batch size are both derived from these so
        they stay consistent.
        """
        max_ploidy = self._max_positive_ploidy(sample_ploidy)
        genotype_classes = max(int(max_ploidy) + 1, 1)
        n_founders = max(int(founder_panel.n_founders), 1)
        state_count = self._state_count_for_memory(n_founders=n_founders, max_ploidy=max_ploidy)
        persistent = self._estimate_io_bytes_per_sample_variant()
        persistent += 4  # dosage matrix (float32)
        if bool(self.config.write_haplotype_probabilities):
            persistent += 4 * n_founders
        if bool(self.config.write_genotype_posteriors) or self.config.pedigree_mode in {"kinship", "transmission"}:
            persistent += 4 * genotype_classes
        # alpha + beta + gamma + emission, each n_positions x state_count float32.
        transient = 16 * int(state_count)
        return int(max(persistent, 1)), int(max(transient, 1))

    def _samples_are_independent(self, founder_panel: FounderPanel) -> bool:
        """True when per-sample HMM results do not depend on other samples.

        Samples are conditionally independent given the founder haplotypes and
        transition parameters, so they can be processed in sample batches only
        when nothing couples them: immutable founders (no EM founder update),
        a single EM iteration, and no transition-factor training.
        """
        return (
            bool(np.all(founder_panel.immutable_mask))
            and int(self.config.em_iterations) == 1
            and not bool(self.config.transition_factor_train)
        )
    ...
    def _build_runtime_memory_plan(
        ...
        *,
        ...
        max_ploidy = self._max_positive_ploidy(sample_ploidy)
        persistent_per_cell, transient_per_cell = self._memory_cell_costs(founder_panel, sample_ploidy)
        samples_independent = self._samples_are_independent(founder_panel)
        configured_batch = int(self.config.jax_sample_batch_size)
        n_samples_pos = max(int(n_samples), 1)
        if configured_batch > 0:
            effective_sample_batch_size = max(1, min(configured_batch, n_samples_pos))
        elif samples_independent:
            # Auto-size the sample batch so the transient forward/backward tensors
            # fit in whatever budget remains after the full-N persistent arrays.
            per_block_budget = max(int(planned_budget) // max(int(effective_block_size), 1), 1)
            avail_for_transient = per_block_budget - n_samples_pos * persistent_per_cell
            auto_batch = avail_for_transient // max(transient_per_cell, 1)
            effective_sample_batch_size = int(min(n_samples_pos, max(1, auto_batch)))
        else:
            # Mutable founders / EM: samples are coupled, cannot batch the HMM.
            effective_sample_batch_size = n_samples_pos
        # Peak working set for one block: full-N persistent arrays + batched transient.
        estimated_peak_bytes = int(
            int(effective_block_size)
            * (n_samples_pos * persistent_per_cell + int(effective_sample_batch_size) * transient_per_cell)
        )
        # Source of truth consumed by _run_hmm_for_subset for serial batching.
        self._serial_sample_batch_size = int(effective_sample_batch_size)
        self._serial_samples_independent = bool(samples_independent)
        ...
        if mode == "exact_streaming":
            notes.append("exact_streaming_preserves_full_snp_span_in_one_hmm_tensor; use sample batching or Dask sample chunks for memory")
        if mode == "exact_chunked":
            notes.append("exact_chunked_preserves_full_snp_span_with_boundary_messages_and_chunkwise_alpha_beta_recompute")
        ...
        return RuntimeMemoryPlan(
            ...
            exact_streaming_requires_boundary_pass=mode == "exact_chunked",
    ...
    def _write_runtime_memory_plan(self, plan: RuntimeMemoryPlan) -> None:
        ...
        xi_manifest = {
            "format": "stitchcont.xi_output_manifest",
    ...
    def _enforce_snp_block_mode(self, plan: RuntimeMemoryPlan) -> None:
        """Refuse to start a run whose estimated peak working set exceeds budget.

        Previously a no-op: the plan estimated multi-TB allocations and wrote them
        to JSON as a "note", then ran anyway and OOMed silently.  We now turn that
        estimate into a hard guard so an unsafe configuration fails fast with
        actionable guidance instead.

        The peak is the HMM working set for ONE block processed at the planned
        sample-batch granularity: effective_sample_batch_size x effective_block_size
        x bytes_per_sample_variant_hmm.  When sample batching/sharding is active the
        batch is small and the run is safe; without it (serial exact_streaming over
        the whole chromosome with all samples) the peak is the full N x M x states
        tensor.
        """
        batch = max(int(plan.effective_sample_batch_size), 1)
        block = max(int(plan.effective_block_size), 1)
        budget = max(int(plan.planned_budget_bytes), 1)
        peak = max(int(plan.estimated_peak_bytes), 1)
        gb = 1024.0 ** 3
        overcommit = bool(getattr(self.config, "allow_memory_overcommit", False))
        ...
        # F9: long-form (non-Zarr) matrix output builds a dense N*M-row table with
        # repeated sample_id/position columns in one allocation.  At whole-chromosome
        # blocks this is billions of rows; refuse early and direct to Zarr.
        if str(self.io_config.output_store) != "zarr":
            long_form_rows = int(plan.n_samples) * int(block)
            long_form_bytes = long_form_rows * 64  # ~ptr+string+pos+dosage per row, lower bound
            if long_form_bytes > budget:
                _raise_or_warn(
                    f"Long-form Parquet output would build ~{long_form_rows:,} rows "
                    f"(~{long_form_bytes / gb:.1f} GB) per block, exceeding the memory budget "
                    f"{budget / gb:.1f} GB. Use --output-store zarr to write dense matrices "
                    "directly (chunked + compressed), or reduce the block/sample count. "
                    "Set --allow-memory-overcommit to bypass."
                )

        if peak <= budget:
            return
        msg = (
            f"Estimated peak working set {peak / gb:.1f} GB exceeds the planned "
            f"memory budget {budget / gb:.1f} GB for snp_block_mode={plan.snp_block_mode!r} "
            f"with sample_batch={batch}, block={block} SNPs, "
            f"n_samples={plan.n_samples}, n_positions={plan.n_positions}. "
            "Reduce the footprint with one or more of: "
            "--jax-sample-batch-size N (HMM-internal sample batching, immutable founders), "
            "--sample-shard-count S / --sample-shard-index I (split samples across runs), "
            "--snp-block-mode independent_approx --block-size B (budget-bounded SNP blocks), "
            "--use-unordered-diploid-states (36 vs 64 states for K=8), "
            "--output-minimal, or raise --max-mem. "
            "Set --allow-memory-overcommit to bypass this guard and accept the OOM risk."
        )
        _raise_or_warn(msg)

    def _apply_sample_shard(self, samples: pd.DataFrame) -> pd.DataFrame:
        """Return the requested sample shard while preserving table order within the shard.

        Samples are conditionally independent given founder haplotypes/transition
        parameters, so immutable-founder production runs can be split exactly by
        sample shard.  The row-modulo scheme is deterministic and does not depend
        on sample IDs being sortable.
        """
        shard_count = int(getattr(self.config, "sample_shard_count", 1) or 1)
        shard_index = int(getattr(self.config, "sample_shard_index", 0) or 0)
        if shard_count <= 1:
            return samples
        if shard_index < 0 or shard_index >= shard_count:
            raise ValueError(f"sample_shard_index must be in [0, {shard_count - 1}], got {shard_index}")
        row_index = np.arange(int(samples.shape[0]), dtype=np.int64)
        mask = (row_index % shard_count) == shard_index
        out = samples.loc[mask].reset_index(drop=True)
        meta = {
            "sample_shard_index": shard_index,
            "sample_shard_count": shard_count,
            "n_samples_total_before_sharding": int(samples.shape[0]),
            "n_samples_in_shard": int(out.shape[0]),
            "scheme": "row_index_mod_sample_shard_count",
        }
        (self.output_dir / "sample_shard.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
        if out.empty:
            raise ValueError(f"Sample shard {shard_index}/{shard_count} is empty.")
        return out

    def _apply_known_founder_fast_path(self, founder_panel: FounderPanel) -> None:
        """Conservatively simplify immutable-founder runs before HMM execution."""
        if not bool(getattr(self.config, "known_founder_fast_path", True)):
            return
        if not bool(np.all(founder_panel.immutable_mask)):
            return
        changes: dict[str, object] = {}
        if int(self.config.em_iterations) != 1:
            changes["em_iterations"] = [int(self.config.em_iterations), 1]
            self.config.em_iterations = 1
        if bool(self.config.transition_factor_train):
            changes["transition_factor_train"] = [True, False]
            self.config.transition_factor_train = False
        if bool(getattr(self.config, "output_minimal", False)):
            if str(self.config.store_xi) != "False":
                changes["store_xi"] = [str(self.config.store_xi), "False"]
                self.config.store_xi = "False"
            if str(self.config.write_gamma) != "off":
                changes["write_gamma"] = [str(self.config.write_gamma), "off"]
                self.config.write_gamma = "off"
            if bool(self.config.write_transition_summary):
                changes["write_transition_summary"] = [True, False]
                self.config.write_transition_summary = False
        if changes:
            (self.output_dir / "known_founder_fast_path.json").write_text(
                json.dumps({"enabled": True, "changes": changes}, indent=2),
                encoding="utf-8",
            )
    ...
    def prepare_inputs(
        ...
        samples = self._apply_sample_shard(samples)
        ...
        self._apply_known_founder_fast_path(founder_panel)
        # IO and HMM configs cache selected output/write flags; refresh after the fast-path simplification.
        self.io_config = self.config.io()
        self._store_xi = str(self.config.store_xi)
    ...
    def _run_hmm_for_subset(
        ...
        *,
        ...
        hmm = self._hmm_runner_for_ploidy(int(ploidy))

        def _run_full(rc, ac, oc, rw, aw, ow, gens, fso, fci, foo, fop, foc, foq):
            return hmm.run(
                founder_panel=founder_panel,
                ref_count=rc,
                alt_count=ac,
        ...
        batch = int(getattr(self, "_serial_sample_batch_size", 0) or 0)
        independent = bool(getattr(self, "_serial_samples_independent", False))
        # Only batch when samples are conditionally independent (immutable founders,
        # no EM founder update / transition-factor training).  Otherwise a single
        # call is required for correctness, and the F1 guard catches the OOM and
        # directs the user to sample sharding.
        if (not independent) or batch <= 0 or batch >= n_samples or n_samples <= 1:
            return _run_full(
                ref_count, alt_count, other_count, ref_weight, alt_weight, other_weight,
                generations, fragment_sample_offsets, fragment_center_idx,
                fragment_obs_offsets, fragment_obs_pos_idx, fragment_obs_code, fragment_obs_qual,
            )

        parts: list[tuple[int, object]] = []
        for s0 in range(0, n_samples, batch):
            s1 = min(s0 + batch, n_samples)
            local = np.arange(s0, s1, dtype=np.int64)
            fso, fci, foo, fop, foc, foq = self._subset_fragments_by_sample_indices(
                local,
            ...
            part = _run_full(
                ref_count[s0:s1], alt_count[s0:s1], other_count[s0:s1],
                ref_weight[s0:s1], alt_weight[s0:s1], other_weight[s0:s1],
                generations[s0:s1], fso, fci, foo, fop, foc, foq,
            )
            parts.append((int(s1 - s0), part))
        return self._concat_hmm_artifacts(parts)

    @staticmethod
    def _concat_hmm_artifacts(parts: list[tuple[int, object]]):
        """Reassemble per-sample HMM artifacts from independent sample batches.

        Per-sample arrays (axis 0 == batch sample count) are concatenated; arrays
        shared across samples (recombination_rate, founder_alt_prob, and any
        position-only transition factors) are taken from the first batch.  Valid
        only when the batches were produced from conditionally-independent samples
        (immutable founders); see _samples_are_independent.
        """
        if len(parts) == 1:
            return parts[0][1]
        first_n, first = parts[0]
        per_sample_fields = (
            "dosage",
            "haplotype_posterior",
            "genotype_posterior",
        ...
        shared_fields = ("recombination_rate", "founder_alt_prob")
        merged: dict[str, object] = {}
        for name in per_sample_fields:
            vals = [getattr(a, name) for _, a in parts]
            if any(v is None for v in vals):
                if not all(v is None for v in vals):
                    raise RuntimeError(f"Inconsistent None for {name!r} across sample batches.")
                merged[name] = None
                continue
            v0 = vals[0]
            # Concatenate only when the first axis is genuinely the sample axis.
            if hasattr(v0, "shape") and len(v0.shape) >= 1 and int(v0.shape[0]) == int(first_n):
                merged[name] = np.concatenate(vals, axis=0)
            else:
                merged[name] = v0
        for name in shared_fields:
            merged[name] = getattr(first, name)
        diagnostics = dict(getattr(first, "em_diagnostics", None) or {})
        diagnostics["serial_sample_batched"] = {
            "n_batches": int(len(parts)),
            "batch_sizes": [int(n) for n, _ in parts],
        }
        return HMMArtifacts(em_diagnostics=diagnostics, **merged)
    ...
    def build_evidence_cache(self, samples: pd.DataFrame) -> dict[str, object]:
        """Extract targeted read/fragment evidence and write the partitioned cache only.

        This is the implementation behind `stitchcont evidence-build`.  It lets
        users pay the BAM/CRAM I/O cost once, then rerun HMM settings from the
        same validated evidence cache using `stitchcont impute-from-evidence`.
        """
        if not self.config.compact_evidence_cache_dir:
            raise ValueError("evidence-build requires --compact-evidence-cache-dir.")
        if str(self.config.compact_evidence_cache_mode) == "read":
            raise ValueError("evidence-build cannot use --compact-evidence-cache-mode read; use write or readwrite.")
        samples = validate_samples(samples)
        samples = self._apply_sample_shard(samples)
        positions_df = load_positions(
            self.config.positions_path,
        ...
        positions_df = self._attach_explicit_genetic_map(positions_df)
        sample_ploidy = self._resolve_sample_ploidy(samples)
        ...
        dummy_founders = FounderPanel(
            chromosome=self.config.chromosome,
            positions=positions_df["POS"].to_numpy(dtype=np.int64, copy=False),
            ref=positions_df["REF"].astype(str).to_numpy(dtype=object),
            alt=positions_df["ALT"].astype(str).to_numpy(dtype=object),
            alt_prob=np.full((max(int(self.config.n_founders), 1), int(positions_df.shape[0])), 0.5, dtype=np.float32),
            immutable_mask=np.zeros(max(int(self.config.n_founders), 1), dtype=bool),
            genetic_cm=(positions_df["CM"].to_numpy(dtype=np.float32, copy=False) if "CM" in positions_df.columns else None),
        )
        ...
        try:
            for block in self._iter_position_blocks_for_mode(positions_df, effective_block_size):
                ...
                evidence, stats = self._extract_or_load_read_evidence(read_samples=read_samples, block=block)
                try:
                    row = {
                        "block_id": int(block.block_id),
                        "row_start": int(block.row_start),
                        "row_stop": int(block.row_stop),
                        "n_positions": int(block.dataframe.shape[0]),
                        "n_read_samples": int(read_samples.shape[0]),
                        "seconds_total": float(time.perf_counter() - b0),
                    }
                        ...
                        row.update(
                            {
                                "compact_evidence_bytes": int(evidence.compact_nbytes),
                                "dense_evidence_bytes": int(evidence.dense_nbytes),
                                "mean_depth": float(np.mean(evidence.depth)) if evidence.depth.size else 0.0,
                                "support_rate": float(np.mean(evidence.depth > 0)) if evidence.depth.size else 0.0,
                                "n_fragments": int(evidence.fragment_center_idx.shape[0]),
                                "n_observations": int(evidence.fragment_obs_pos_idx.shape[0]),
                            }
                        )
                    row.update(stats)
                    timings.append(row)
        ...
        timing_path = self.output_dir / "evidence_build_timings.parquet"
        if timings:
            write_parquet(
                pa.Table.from_pandas(pd.DataFrame(timings), preserve_index=False),
                timing_path,
                self.config.compression,
                self.config.compression_level,
            )
        summary = {
            ...
            "compact_evidence_cache_dir": str(self.config.compact_evidence_cache_dir),
            "compact_evidence_cache_mode": str(self.config.compact_evidence_cache_mode),
            "n_samples": int(samples.shape[0]),
            "n_read_samples": int(read_samples.shape[0]),
            "n_positions": int(positions_df.shape[0]),
            "n_blocks": int(len(timings)),
            "elapsed_seconds": float(time.perf_counter() - t0),
            "timings": str(timing_path) if timings else None,
        }
        (self.output_dir / "evidence_build_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
        return summary
    ...
    def _run_blocks(
        ...
        self._output_sample_ids = sample_ids.astype(object, copy=True)
        self._output_positions = positions_df["POS"].to_numpy(dtype=np.int64, copy=True)
        sample_ploidy = (
    ...
    def _run_blocks_dask(
        ...
        try:
            ...
            raise ImportError(
                "The Dask executor requires dask. Install stitchcont with the Dask dependencies."
            ) from exc
        ...
        if str(self.config.snp_block_mode) in {"exact_streaming", "exact_chunked"} and effective_block_size < int(positions_df.shape[0]):
            raise NotImplementedError(
                ...
                "or run serial exact_streaming/exact_chunked with a single planned block until the boundary streaming "
        ...
        if scheduler_kind == "local":
            ...
            try:
                ...
                raise ImportError(
                    "The jobqueue scheduler requires dask-jobqueue. Install stitchcont with the jobqueue extras."
                ) from exc
        ...
        dask_summary = {
            ...
            "snp_block_mode_approximate": str(self.config.snp_block_mode) not in {"exact_streaming", "exact_chunked"},
    ...
    @staticmethod
    def _zarr_edge_shape(shape: tuple[int, ...], chunks: tuple[int, ...], chunk_index: tuple[int, ...]) -> tuple[int, ...]:
        out = []
    ...
    @staticmethod
    def _zarr_chunk_path(array_dir: Path, chunk_index: tuple[int, ...]) -> Path:
        return array_dir / ".".join(str(int(i)) for i in chunk_index)

    def _ensure_zarr_array(
        self,
        name: str,
        *,
        shape: tuple[int, ...],
        ...
        root = self.output_dir / "matrices.zarr"
        root.mkdir(parents=True, exist_ok=True)
        (root / ".zgroup").write_text(json.dumps({"zarr_format": 2}, sort_keys=True), encoding="utf-8")
        root_attrs_path = root / ".zattrs"
        if not root_attrs_path.exists():
            root_attrs_path.write_text(
                json.dumps(
                    {
        ...
        if self._output_sample_ids is not None:
            (root / "sample_ids.json").write_text(
                json.dumps([str(x) for x in self._output_sample_ids.tolist()], indent=2),
                encoding="utf-8",
            )
        if self._output_positions is not None:
            np.save(root / "positions.npy", np.asarray(self._output_positions, dtype=np.int64))
        array_dir = root / name
        ...
        zarray = {
            ...
            "compressor": (None if str(getattr(self.io_config, "zarr_compressor", "none")).lower() in {"", "none", "null"} else {"id": "zlib", "level": int(getattr(self.io_config, "zarr_compression_level", 1))}),
        ...
        if zarray_path.exists():
            existing = json.loads(zarray_path.read_text(encoding="utf-8"))
            if tuple(existing.get("shape", ())) != tuple(zarray["shape"]):
                raise ValueError(f"Existing Zarr array {array_dir} shape {existing.get('shape')} != requested {zarray['shape']}")
            if tuple(existing.get("chunks", ())) != tuple(zarray["chunks"]):
                raise ValueError(f"Existing Zarr array {array_dir} chunks {existing.get('chunks')} != requested {zarray['chunks']}")
        else:
            zarray_path.write_text(json.dumps(zarray, indent=2, sort_keys=True), encoding="utf-8")
        zattrs = dict(attrs or {})
        zattrs["_ARRAY_DIMENSIONS"] = list(dims)
        (array_dir / ".zattrs").write_text(json.dumps(zattrs, indent=2, sort_keys=True, default=str), encoding="utf-8")
        self._output_zarr_manifest[name] = {
            "path": str(array_dir),
            "shape": [int(x) for x in shape],
            "chunks": [int(x) for x in chunks],
        ...
        (root / "stitchcont_matrix_manifest.json").write_text(
            json.dumps(self._output_zarr_manifest, indent=2, sort_keys=True),
            encoding="utf-8",
        )
        return array_dir

    def _write_zarr_matrix_block(
        self,
        name: str,
        data: np.ndarray,
        *,
        ...
        if arr.ndim not in {2, 3}:
            raise ValueError(f"Zarr matrix output supports 2D/3D arrays, got {name} with shape {arr.shape}")
        if self._output_sample_ids is None:
            n_samples_total = int(arr.shape[0])
        else:
            n_samples_total = int(self._output_sample_ids.shape[0])
        if self._output_positions is None:
            n_positions_total = int(pos_start + arr.shape[1])
        else:
            n_positions_total = int(self._output_positions.shape[0])
        if arr.shape[0] != n_samples_total:
            raise ValueError(f"{name} sample dimension {arr.shape[0]} != run sample count {n_samples_total}")
        shape = (n_samples_total, n_positions_total) + tuple(arr.shape[2:])
        chunk_samples = min(max(int(self.io_config.zarr_chunk_samples), 1), max(n_samples_total, 1))
        chunk_positions = min(max(int(self.io_config.zarr_chunk_positions), 1), max(n_positions_total, 1))
        chunks = (chunk_samples, chunk_positions) + tuple(arr.shape[2:])
        array_dir = self._ensure_zarr_array(
            name,
            shape=shape,
            chunks=chunks,
        ...
        local_pos_stop = int(pos_start) + int(arr.shape[1])
        for sample_start in range(0, n_samples_total, chunk_samples):
            sample_stop = min(sample_start + chunk_samples, n_samples_total)
            local_sample_slice = slice(sample_start, sample_stop)
            for global_pos_chunk_start in range((int(pos_start) // chunk_positions) * chunk_positions, local_pos_stop, chunk_positions):
                global_pos_chunk_stop = min(global_pos_chunk_start + chunk_positions, n_positions_total)
                overlap_start = max(int(pos_start), global_pos_chunk_start)
                overlap_stop = min(local_pos_stop, global_pos_chunk_stop)
                if overlap_start >= overlap_stop:
                    continue
                chunk_index = (sample_start // chunk_samples, global_pos_chunk_start // chunk_positions) + tuple(0 for _ in arr.shape[2:])
                chunk_shape = self._zarr_edge_shape(shape, chunks, chunk_index)
                chunk_path = self._zarr_chunk_path(array_dir, chunk_index)
                if chunk_path.exists():
                    raw = chunk_path.read_bytes()
                    if str(getattr(self.io_config, "zarr_compressor", "none")).lower() == "zlib":
                        raw = zlib.decompress(raw)
                    chunk_arr = np.frombuffer(raw, dtype=np.dtype(dtype)).copy().reshape(chunk_shape)
                else:
                    chunk_arr = np.full(chunk_shape, fill_value, dtype=np.dtype(dtype))
                sample_len = sample_stop - sample_start
                chunk_pos0 = overlap_start - global_pos_chunk_start
                chunk_pos1 = overlap_stop - global_pos_chunk_start
                data_pos0 = overlap_start - int(pos_start)
                data_pos1 = overlap_stop - int(pos_start)
                if arr.ndim == 2:
                    chunk_arr[:sample_len, chunk_pos0:chunk_pos1] = arr[local_sample_slice, data_pos0:data_pos1]
                else:
                    chunk_arr[:sample_len, chunk_pos0:chunk_pos1, :] = arr[local_sample_slice, data_pos0:data_pos1, :]
                raw_out = np.ascontiguousarray(chunk_arr).tobytes(order="C")
                if str(getattr(self.io_config, "zarr_compressor", "none")).lower() == "zlib":
                    raw_out = zlib.compress(raw_out, level=max(0, min(int(getattr(self.io_config, "zarr_compression_level", 1)), 9)))
                chunk_path.write_bytes(raw_out)

    def _quantize_zarr_probability(self, arr: np.ndarray) -> tuple[np.ndarray, np.dtype, object, dict[str, object]]:
        mode = str(getattr(self.io_config, "zarr_gp_dtype", "float32")).lower()
        if mode == "float16":
            return np.asarray(arr, dtype=np.float16), np.dtype(np.float16), float("nan"), {"storage_dtype": "float16"}
        if mode == "uint16":
            scale = max(int(getattr(self.io_config, "zarr_probability_scale", 65535)), 1)
            q = np.rint(np.clip(np.asarray(arr, dtype=np.float32), 0.0, 1.0) * float(scale)).astype(np.uint16)
            return q, np.dtype(np.uint16), 0, {"storage_dtype": "uint16", "probability_scale": scale, "decode": "probability = stored / probability_scale"}
        return np.asarray(arr, dtype=np.float32), np.dtype(np.float32), float("nan"), {"storage_dtype": "float32"}

    def _quantize_zarr_dosage(self, arr: np.ndarray) -> tuple[np.ndarray, np.dtype, object, dict[str, object]]:
        mode = str(getattr(self.io_config, "zarr_dosage_dtype", "float32")).lower()
        if mode == "float16":
            return np.asarray(arr, dtype=np.float16), np.dtype(np.float16), float("nan"), {"storage_dtype": "float16"}
        return np.asarray(arr, dtype=np.float32), np.dtype(np.float32), float("nan"), {"storage_dtype": "float32"}
    ...
    def _write_hmm_outputs(
        ...
        *,
        ...
        use_zarr_matrix_output = str(self.io_config.output_store) == "zarr"
        if use_zarr_matrix_output:
            dosage_z, dosage_dtype, dosage_fill, dosage_attrs = self._quantize_zarr_dosage(dosage)
            self._write_zarr_matrix_block(
                "dosage",
                dosage_z,
                pos_start=int(block.row_start),
                dtype=dosage_dtype,
                dims=("sample", "position"),
                fill_value=dosage_fill,
                attrs={"description": "ALT dosage", "chromosome": str(self.config.chromosome), **dosage_attrs},
            )
        else:
        ...
        if self.io_config.write_support_mask:
            ...
            if use_zarr_matrix_output:
                if bool(getattr(self.io_config, "zarr_bitpack_support_mask", False)):
                    packed = np.packbits(support_mask.astype(np.uint8, copy=False), axis=1, bitorder="little")
                    packed_dir = self.output_dir / "support_mask_bitpacked"
                    packed_dir.mkdir(parents=True, exist_ok=True)
                    np.savez_compressed(
                        packed_dir / f"block={block.block_id:06d}.npz",
                        packed=packed.astype(np.uint8, copy=False),
                        positions=positions.astype(np.int64, copy=False),
                        sample_ids=np.asarray(sample_ids, dtype=object),
                        bitorder=np.asarray(["little"], dtype=object),
                        n_positions=np.asarray([int(n_positions)], dtype=np.int64),
                    )
                    (packed_dir / f"block={block.block_id:06d}.json").write_text(
                        json.dumps(
                            {
                                "format": "stitchcont.support_mask_bitpacked.v1",
                                "block_id": int(block.block_id),
                                "row_start": int(block.row_start),
                                "row_stop": int(block.row_stop),
                                "n_samples": int(n_samples),
                                "n_positions": int(n_positions),
                                "packed_shape": [int(x) for x in packed.shape],
                                "bitorder": "little",
                                "decode": "np.unpackbits(packed, axis=1, bitorder='little')[:, :n_positions]",
                            },
                            indent=2,
                            sort_keys=True,
                        ),
                    ...
                    self._write_zarr_matrix_block(
                        "support_mask",
                        support_mask.astype(np.bool_, copy=False),
                        pos_start=int(block.row_start),
                        dtype=np.dtype(np.bool_),
                        dims=("sample", "position"),
                        fill_value=False,
                        attrs={"description": "Whether a sample/SNP cell has direct read support", "chromosome": str(self.config.chromosome)},
                    )
            else:
        ...
        if self.io_config.write_haplotype_probabilities:
            ...
            if use_zarr_matrix_output:
                self._write_zarr_matrix_block(
                    ...
                    dims=("sample", "position", "founder"),
                    fill_value="NaN",
                    attrs={"description": "Founder haplotype dosage", "chromosome": str(self.config.chromosome), "n_founders": int(k)},
                )
                self._write_zarr_matrix_block(
                    "haplotype_probability",
                    hap_probability,
                    pos_start=int(block.row_start),
                    dtype=np.dtype(np.float32),
                    dims=("sample", "position", "founder"),
                    fill_value="NaN",
                    attrs={"description": "Founder haplotype posterior probability", "chromosome": str(self.config.chromosome), "n_founders": int(k)},
                )
            else:
        ...
        if self.io_config.write_genotype_posteriors:
            ...
            if use_zarr_matrix_output:
                gp_z, gp_dtype, gp_fill, gp_attrs = self._quantize_zarr_probability(genotype_posterior)
                self._write_zarr_matrix_block(
                    "genotype_posterior",
                    gp_z,
                    pos_start=int(block.row_start),
                    dtype=gp_dtype,
                    dims=("sample", "position", "genotype_class"),
                    fill_value=gp_fill,
                    attrs={"description": "Genotype posterior probabilities", "chromosome": str(self.config.chromosome), **gp_attrs},
                )
            else:
                gp_values = pa.array(
        ...
        if self.io_config.write_genotype_calls:
            ...
            if use_zarr_matrix_output:
                self._write_zarr_matrix_block(
                    "genotype_call",
                    genotype_call.astype(np.int8, copy=False),
                    pos_start=int(block.row_start),
                    dtype=np.dtype(np.int8),
                    dims=("sample", "position"),
                    fill_value=-1,
                    attrs={"description": "Hard genotype call; -1 indicates no-call", "chromosome": str(self.config.chromosome)},
                )
            else:
                gt_table = pa.table(
                    {
